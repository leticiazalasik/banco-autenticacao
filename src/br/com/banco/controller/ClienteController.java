...
public class ClienteController {
	...
	public boolean validarSatusConta(int id) {
		try {
			ClienteDAOImpl dao = new ClienteDAOImpl();
			

			if (dao.verificarStatusConta(id)==true) {
				return true;
			} else {
				return false;
			}
	...
	public Boolean fecharConta (int id) throws Exception { 
		ClienteDAOImpl dao = new ClienteDAOImpl(); 

		try { 			
			if (validarId(id)==false || validarSatusConta(id)==false) { 
			JOptionPane.showMessageDialog(null, "Não foi possível encerrar a conta. \n Ou cliente não existe ou a conta já está inativa.");
			return false; 
